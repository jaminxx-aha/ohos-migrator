...
Rules:
- oldText MUST be an exact, CONTIGUOUS substring that appears exactly once in the file — it cannot skip over unrelated lines in the middle. If you need to change two non-adjacent spots (e.g. add an import at the top AND change a call site further down), emit SEPARATE edits: one whose oldText anchors on the import line, another whose oldText is the call site.
...
- Edit ONLY the deprecated call sites listed in 'Remaining deprecated call sites'. You may ADD a new import line for a replacement symbol (e.g. a new 'import X from @ohos.kit'), but you MUST NOT modify any existing import line — do not change the module specifier or bindings of an import already in the file (e.g. do NOT rewrite 'import { Context } from @ohos.abilityAccessCtrl' to point at another kit). Existing imports are not deprecated call sites; changing them breaks the build. To ADD an import next to an existing one, set oldText to that existing import line and newText to the same line followed by your new import line (the existing import's specifier and bindings stay byte-for-byte unchanged).
- New import lines you ADD MUST be placed at the top of the file, immediately after the last existing import — NEVER after a declare/const/let/function/expression statement. ArkTS forbids imports after other statements (arkts-no-misplaced-imports); a misplaced import fails compilation even though it type-checks.
