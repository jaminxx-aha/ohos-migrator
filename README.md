...
3. 逐文件交 agent：把废弃清单 + 文件内容给模型，模型调 `edit_file` / `replace_file` 改、`list_deprecated` 自检、`done` 收尾（单轮 ≤ 25 步）。**失败不从头来，同一会话内继续**：
   - `done` 触发 hvigor 编译门禁：编译干净才真正结束；有编译错误则 `done` 被拒、错误清单喂回 agent，在同一轮里继续 `edit_file` 定点修，修完再 `done`。
   - 流式超时 / 网络中断：不结束对话、不丢已做的编辑，告诉 AI「超时，接着改」并在同一会话重试（连续 3 次才放弃）。
   - 模型只输出文字不调工具：提示其继续用工具（连续 2 次才放弃）。
   - 整轮 ≤ 5 次重试；编译错误重试时保留上一轮 agent 产出做定点修（不回滚原文，避免「文件 BY、错误说 On」自相矛盾）。
