...
**分类现状**（API 24 真实 map 4713 条，带 @useinstead 3891 条；2026-08-06 精算，口径=镜像 scanner/rewriter 实际分类）：auto/covered 2676 条（56.9%）；manual 2037 条（43.1%：no_replacement 834、chain-length-mismatch 602、cross-kit 565、export-not-indexed 35、kit-not-indexed 1）。两轮冗余抑制已落地：①自指 no-op（sameKit 且链全等，原 90 条）归 rewrite-import suppressed 不报；②已被 crossKitDropin specifier 改写覆盖、链相同的成员（原 11 条：Configuration.language/.colorMode + 9 个 Want 成员）在 scanProjectMembers 抑制。剩余 manual 大头经深挖确认为调用形状真变化、不可安全自动替换：cross-kit 565 多为 namespace 改名但成员级废弃（alias 整 binding 会误伤非废弃成员）；chain-length-mismatch 602 多为枚举↔枚举成员等语义变化（如 `Event`→`Event.USER_LOGIN`、`AbilityLifecycleState.UNINITIALIZED`→`AbilityLifecycleState`）；no_replacement 834 无 @useinstead。
...
测试：`npm test`（node --test，120 个，全部通过）。SDK 路径见 [[harmonyos-sdk-location]]。
